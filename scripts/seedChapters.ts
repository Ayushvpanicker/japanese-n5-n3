...
export const CHAPTER_26_50_VOCABULARY = [
  // --- CHAPTER 26 ---
  { chapter: 26, kanji: '診ます', reading: 'みます', meaning: 'check, examine', notes: '病気を〜' },
  ...
  { chapter: 26, kanji: '出す', reading: 'だす', meaning: 'put out [the trash]', notes: 'ごみを〜' },
...
async function seedData() {
  console.log(`\n🚀 Seeding ${CHAPTER_26_50_VOCABULARY.length} curated cards into Supabase...`);

  const { data, error } = await supabase.from('cards').insert(CHAPTER_26_50_VOCABULARY).select();

  if (error) {
    console.error('❌ Insert error:', error.message);
    if (error.message.includes("Could not find the table 'public.cards'")) {
      console.error('\n💡 HINT: Create the "cards" table in Supabase SQL Editor first!');
    }
  } else {
    console.log(`🎉 Success! Inserted ${data ? data.length : CHAPTER_26_50_VOCABULARY.length} cards into Supabase cards table!\n`);
  }
}
